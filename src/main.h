...
static const int COINBASE_MATURITY_PPC = 240;
...
static const int STAKE_MIN_AGE = 600;
...
public:
    ...
    {
        // SPOTS: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // SPOTS: get transaction coin age
...
public:
    ...
    // SPOTS: block signature - signed by coin base txout[0]'s owner
    ...
    // SPOTS: two types of block: proof-of-work or proof-of-stake
    ...
    // SPOTS: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // SPOTS: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // SPOTS: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // SPOTS: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // SPOTS: block index flags
    enum  
    {
