...
// SPOTS: optional setting to unlock wallet for block minting only;
...
// SPOTS: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // SPOTS: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // SPOTS: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // SPOTS: not avatar mode
                        {
...
// SPOTS: create coin stake transaction
...
// SPOTS: check 'spent' consistency between wallet and txindex
// SPOTS: fix wallet spent state according to txindex
...
// SPOTS: disable transaction (only for coinstake)
